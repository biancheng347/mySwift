//
//  CollectionView+Index.swift
//  vskit
//
//  Created by jackwang on 2025/5/28.
//
...
private extension LXMCollectionIndexView {
    ...
    func scrollCollectionView_old(toTextLayer textLayer: LXMTextLayer, animated: Bool) {
    ...
    func scrollCollectionView(toTextLayer textLayer: LXMTextLayer, animated: Bool) {
        ...
        // 修复：考虑 contentInset 的 left 值
        let contentInsetLeft = collectionView.contentInset.left
        ...
        let cellAttributes = collectionView.layoutAttributesForItem(at: indexPath) {
            // 应用 contentInset.left
            var targetPoint = CGPoint(
                x: -contentInsetLeft, // 确保左侧间距生效
                y: cellAttributes.frame.origin.y - attributes.frame.size.height
            )
            
            // 限制在合法范围内
            let minY = -collectionView.contentInset.top
            let maxY = max(0, collectionView.contentSize.height - collectionView.bounds.height + collectionView.contentInset.bottom)
            targetPoint.y = min(maxY, max(minY, targetPoint.y))
            
            collectionView.setContentOffset(targetPoint, animated: animated)
        } else {
            // 备用方法同样应用 contentInset
...
public class LXMCollectionIndexViewConfiguration: NSObject {
    ...
    var itemTextColor: UIColor = 0xB168FF.color
    var itemSelectedBackgroundColor: UIColor = .gray
    var itemSelectedTextColor: UIColor =  .white
...
func generateAlphabetWithHash() -> [String] {
    // 生成A-Z的字母数组
